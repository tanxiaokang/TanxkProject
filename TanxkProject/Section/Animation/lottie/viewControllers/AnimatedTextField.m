...
- (void)changeCharactersInRange:(NSRange)range
  ...
  [_collectionView performBatchUpdates:^{
    if (addIndices) {
        [self->_collectionView insertItemsAtIndexPaths:addIndices];
    }
    if (updateIndices) {
        [self->_collectionView reloadItemsAtIndexPaths:updateIndices];
    }
    if (removeIndices) {
        [self->_collectionView deleteItemsAtIndexPaths:removeIndices];
    }
  } completion:^(BOOL finished) {
      self->_updatingCells = NO;
  }];
  [self scrollToBottom];
}
